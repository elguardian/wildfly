...
/**
 * Reproducer for WFCORE-4668: transaction recovery of remote EJB calls should not use the static
 * default AuthenticationContext.
 *
 * <p>Scenario: the outbound server calls a remote EJB on the inbound server with transaction
 * propagation, then the outbound server JVM crashes during 2PC prepare. The inbound server has
 * already prepared its XA resource. On restart, the outbound server must recover the transaction
 * by reconnecting to the inbound server.</p>
 *
 * <p>The inbound server requires DIGEST-MD5 authentication (no anonymous). The outbound server
 * has an {@code authentication-context} for normal EJB calls, but {@code default-authentication-context}
 * is NOT set on the elytron subsystem. When recovery runs, it constructs a new
 * {@link org.wildfly.transaction.client.provider.remoting.RemotingRemoteTransactionPeer} which
 * calls {@code AuthenticationContext.captureCurrent()} — on the recovery thread this returns an
 * empty context, causing the reconnection to fail and leaving the transaction unresolved.</p>
 *
 * <p>This test is expected to FAIL until WFCORE-4668 is fixed. Once a
 * {@code default-authentication-context-for-recovery} attribute (or equivalent) is added to the
 * elytron subsystem and wired into the WFTC recovery path, the test should pass.</p>
 */
...
public class RecoveryWithAuthContextTestCase {
    ...
    private static final String CLIENT_DEPLOYMENT = "wfcore4668-client";
    private static final String SERVER_DEPLOYMENT = "wfcore4668-server";
    ...
    private static final String PROPERTIES_REALM   = "Wfcore4668Realm";
    private static final String SECURITY_DOMAIN    = "Wfcore4668Domain";
    private static final String SASL_FACTORY       = "Wfcore4668SaslFactory";
    private static final String EJB_APP_DOMAIN     = "Wfcore4668Domain";
    private static final String INBOUND_CONNECTOR  = "wfcore4668-connector";
    private static final String INBOUND_SOCKET     = "wfcore4668-socket";
    ...
    private static final String AUTH_CONFIG         = "Wfcore4668AuthConfig";
    private static final String AUTH_CONTEXT        = "Wfcore4668AuthContext";
    private static final String OUTBOUND_SOCKET     = "wfcore4668-inbound-binding";
